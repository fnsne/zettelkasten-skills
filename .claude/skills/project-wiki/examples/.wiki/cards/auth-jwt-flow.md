...
的 stateless token。根據 [[decision-jwt-vs-session|當初的決策]],本系統採
stateless 設計而非 server-side session。

## 內容
登入請求依 [[architecture-auth-layer|auth layer 規範]],先由 API gateway
驗證請求格式,再交給 auth service 處理。auth service 從資料庫驗證帳密後,
組裝 payload(uid, role, exp)並用 RS256 簽發。

簽發後的 token 狀態追蹤交給 [[auth-session-mgmt]] 負責,並搭配
[[security-token-rotation|token rotation 策略]]降低洩漏風險——
refresh token 一次性使用,使用後立即作廢。

## 何時往下追
- 為什麼選 JWT,不選 session → [[decision-jwt-vs-session]]
- session / refresh token 怎麼配合 → [[auth-session-mgmt]]
- rotation 策略細節 → [[security-token-rotation]]
