...
**Rule P7 — Replace, don't append (no inline changelog).** When a decision or fact changes, rewrite the statement to the **current** truth; keep the old value only on a trailing `落點：` / `變更：` line if it's worth keeping. Never carry a superseded decision inside the body prose — no strikethrough mid-sentence, no 「原記 X；YYYY-MM-DD 推翻改 Y」 clauses woven into a sentence. The body must always read as current state; history lives on a trailing line, not inside the point. This is the single biggest source of unreadable long cards: they decay by gluing each new decision onto the old one, so the reader has to reconstruct "what's true now" from an inline diff — and Claude, reading it later, can report a superseded decision as current.

> P7 is about *recording* a change you already know about — how to write it once. It does **not** cover *discovering* that a card's facts have silently gone stale against the code (e.g. a card still says "未實作" for a status the enum now contains). That is a separate verification concern, handled by the audit's **"claims stale vs. current source"** check — do not fold it into P7 or the prose readback.
...
4. **Batch-write every approved item as a draft to `wiki/_drafts/` (one generation pass).** Write them **all** in this pass, in dependency order — the user waits exactly once. **Do not touch `cards/` at all in this step.** Every draft carries `status: draft` in frontmatter and ends with the pre-seeded REVIEW template (below), so the user never has to remember or type the markers.
   ...
   **Readability readback before writing any `[NEW]` / `[EXPAND]` draft (Prose style P1–P7).** In the same pass, re-read the body once *as a zero-context reader* and fix: sentences carrying more than one fact or nested parentheticals (P1); points buried behind inline ids/dates/caveats instead of leading with meaning and trailing the refs (P2); un-expanded session shorthand or code-names (P3); more than one bold per bullet (P4); a 摘要 opening sentence that isn't the plainest line on the card (P5); superseded decisions carried inline (strikethrough / 「原記…推翻改…」) instead of the body stating current truth with history on a trailing line (P7). Rewrite before staging — this readback is the writer's job (P6), so the user reviews meaning, not prose density.
...
**Claude-side check (semantic): prose readability (Prose style P1–P7).** Claude reads card bodies for the readability failures no script can see: sentences packing multiple facts or nested parentheticals (P1); points buried under inline ids/dates/caveats instead of leading with meaning (P2); un-expanded session shorthand or code-names (P3); bold on everything (P4); a 摘要 whose first sentence isn't the plainest line on the card (P5); a body that carries superseded decisions inline (strikethrough / 「原記…推翻改…」 changelog) instead of stating current truth and logging history on a trailing line (P7). Report each as a rewrite candidate with a proposed plainer version. **Be conservative** — a dense-but-clear sentence is fine; flag only prose a zero-context reader would have to decode or reflow. Fix via a direct edit or by re-invoking `extract` on the card's source. This is how the *already-written* dense cards get cleaned up over time, not just new drafts.

**Claude-side check (semantic): claims stale vs. current source.** The deterministic `stale` check (check 4) only compares file mtimes — it says a source *changed*, not *what* the card now gets wrong. This check goes further: for a card whose `sources:` point to code or spec files, read the source at the referenced lines and spot-check the card's concrete, verifiable claims against it — especially 已實作／未實作 status, status/field/enum names, transitions, and specific values. Flag any claim the current source contradicts as a correction candidate (e.g. a card saying a status is 未實作 when the enum already contains it). **This is distinct from the prose rules (P1–P7):** P1–P7 govern how a card *reads* and how a change you *already know about* is recorded (P7 = replace, don't append); this check is what *discovers* that a card's facts have gone stale against the code. Never auto-rewrite — surface the mismatch, let the author confirm, and when correcting apply P7 (state current truth, log the old value on a trailing 變更 line). Be conservative: verify against the source before flagging, and only flag claims the source actually contradicts, not ones it merely doesn't mention.
